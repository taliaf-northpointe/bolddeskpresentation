...
const T = {
  ...
  clickup:  11.45,   // "with ClickUp"                  -> Project work comes forward
  tick:     12.72,   // "A ticket"                      -> the checkbox ticks
  push:     13.37,   // "pushed into ClickUp"           -> it crosses
  task:     14.75,   // "as a project task"             -> the task card appears
  fields:   16.03,   // "information carried over"      -> fields fill in
  number:   19.32,   // "The ClickUp number"            -> TASK-318
  linkBack: 19.92,   // "and link can then come back"   -> link chip travels back
  onTicket: 21.54,   // "into the BoldDesk ticket"      -> lands as a linked row
  keep:     22.94,   // "keeping the two systems connected" -> connecting line stays
  tools:    29.28,   // "connect the tools teams already use" -> unnamed tools link in
  dupIn:    31.39,   // "instead of making people duplicate" -> "typed twice" ghost
  dupOut:   32.78,   // "duplicate"                     -> and it fades
  autoIn:   36.31,   // "opportunities for automated"   -> Automation panel forward
  chips: [37.85, 38.93, 39.48, 40.39, 42.33],   // responses, routing, assignment, document, repetitive
  recede:   44.51,   // "every time we automate"        -> chips and cards recede
  person:   45.38,   // "automate something repetitive" -> figure steps forward
  giving:   47.40,   // "giving people back time"       -> the clock winds back
  line:     48.73,   // "to work on the things..."      -> on-screen line
};
