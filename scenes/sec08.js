...
const T = {
  ...
  q5:       18.71,   // "Are requests being answered within…"  -> Response within target
  q6:       22.09,   // "And are they being resolved within…"  -> Resolution within target
  armDown:  25.65,   // "That visibility gives managers"    -> arm relaxes
  line1:    29.74,   // "a clear picture"                   -> on-screen line
  line1Out: 41.50,   // just before the dashboard rises
  spot:     33.66,   // "bottlenecks"                       -> one bar turns amber
  issues:   34.48,   // "workload issues"                   -> second nudge
  spread:   37.01,   // "move resources"                    -> the work redistributes
  settled:  38.40,   // "needed"                            -> amber gone
  shrink:   41.75,   // "establish measurable"              -> dashboard shrinks and rises
  optA:     45.01,   // "Maybe that's a common standard"    -> option card A
  optB:     48.25,   // "Maybe each team has its own SLA"   -> option card B
  check:    53.79,   // "either approach"                   -> a check on BOTH at once
  back:     55.24,   // "The important part"                -> dashboard returns behind
  line2:    57.54,   // "actually see the results"          -> on-screen line
};
